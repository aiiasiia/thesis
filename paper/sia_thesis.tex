...
\FirstPanelMember{XXX, Ph.D.}	% Doc Andrei Coronel
\SecondPanelMember{XXX, Ph.D.}	% Ma'am Juliet
\ThirdPanelMember{XXX, Ph.D.}	% Jal de Vera
...
\chapter{Introduction}
	...
	\section{Context of the Study}
		...
		\noindent\textbf{\boldmath DEFINITION 3. $d$-neighborhood}
		...
		\noindent \hspace*{35pt} Ex. \texttt{\ul{cg}tta\ul{g}a} is a $d$-neighbor of \texttt{\ul{ga}tta\ul{c}a} for any $d \geq 3$.\newline
		...
		\noindent\begin{minipage}{\textwidth} {\setstretch{1.0}
				\noindent\hspace*{35pt} Ex. $N(\texttt{gatta}, 1) =$\{ \texttt{gatta},
					...
					\hspace*{145pt}\texttt{g\ul{c}tta}, \texttt{g\ul{g}tta}, \texttt{g\ul{t}tta},
					% \hspace*{145pt}
					\texttt{ga\ul{a}ta}, \texttt{ga\ul{c}ta}, \texttt{ga\ul{g}ta},

				 	\hspace*{145pt}\texttt{gat\ul{a}a}, \texttt{gat\ul{c}a}, \texttt{gat\ul{g}a},
				 	% \hspace*{145pt}
...
\chapter{Review of Related Literature}
	...
	\section{Heuristic Algorithms}
		...
		////// list more algorithms! name, citation

	\section{Exact Algorithms}
		While heuristics can be efficient, these approaches are non-exhaustive and thus do not always guarantee to find a solution. Exact motif search algorithms, on the other hand, perform an exhaustive search of possible motifs and so always find the planted motif.
		...
		WINNOWER \cite{pevzner2000combinatorial} and its successor MITRA \cite{eskin2002finding} are exact algorithms that look at pairwise $l$-mer similarity to find motifs. In a set of DNA sequences, there are numerous pairs of ``similar'' $l$-mers, which come from different sequences and have Hamming distances of at most 2$d$ from each other (meaning that they could possibly be two $d$-neighbors of the same $l$-mer). WINNOWER represents these pairs in a graph, with $l$-mers as nodes and edges connecting $l$-mer pairs. It then prunes the graph to identify ``cliques'' of pairs that indicate a motif. MITRA refines this graph representation into a mismatch tree containing all possible $l$-mers, organized by prefix. The tree structure allows MITRA to eliminate entire branches at a time, making it significantly faster than WINNOWER at removing the many spurious edges that are not part of any motif clique.
		 ...
		 ////// add photo for MITRA, with citation
	...
	\subsection{PMS8 and qPMS9}
		...
		\cite{pms2007,pms2014,pms2015}. The idea in both PMS8 and qPMS9 is to first consider a subset of $\mathcal{S}$, $S_1,...S_{n'}$, and from these, form all possible $n'$-tuples of ``similar'' $l$-mers (similarity is defined in Step 1 below); then, determine whether the common neighbors of $l$-mers in these tuples are motifs. This approach proceeds in two main steps:
		...
		\begin{enumerate}
			...
			\begin{equation} M =\{ c \in C\ |\ \forall i \in \{n'+1,...,n\}\ \exists\ x_i \in \mathcal{L}(S_i, l)\ \text{such that}\ dH(c, x_i) \leq d \}. \end{equation}
			...
			\begin{algorithmic}[1]
				...
				\Procedure{AddNeighbors}{$x$, $s$, $d$}
					\For{$i \leftarrow s$ to $l$}
						\State $\Sigma' \leftarrow$ \{\texttt{a}, \texttt{g}, \texttt{c}, \texttt{t}\} - $\{x[i]\}$ \hspace*{79pt}\Comment{\small remove $i^{th}$ character of $x$}
...
\chapter{Results and Analysis}
	...
	\section{Performance of improved EMS-GT}
		...
		EMS-GT is highly competitive with PMS8 and qPMS9 on challenging problem instances, but as Table 4.\ref{tbl:runtimes_nonchallenge} shows, on non-challenging instances EMS-GT is not as efficient. This may be due to differing ways of narrowing down the search space of the search space: while EMS-GT manipulates a fixed-size bit-array to narrow down the search space of all $4^l$ $l$-mers, PMS8 and qPMS9 have a dynamically-sized search space which is easily pruned down for most ($l$,$d$) values, but may be more difficult to manage for challenging ($l$,$d$) values.
